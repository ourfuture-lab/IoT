...
String ESP8266::recvString(String target, uint32_t timeout)
{
    ...
    while (millis() - start < timeout) {
        ...
        if (data.indexOf(target) != -1) {
            //Serial.println("***recvString:indexof ok***");
            break;
        }   
    }
    Serial.println("\r\n-----------------------------recvString_target1:");
...
bool ESP8266::recvFindAndFilter(String target, String begin, String end, String &data, uint32_t timeout)
{
    ...
    //Serial.print("recvFindAndFilter***:");
    //Serial.println(data_tmp);
    
    if (data_tmp.indexOf(target) != -1) {
        //Serial.println("\r\n***recvFindAndFilter:indexof ok***:");
...
bool ESP8266::eATGMR(String &version)
{
    ...
    return recvFindAndFilter("version", "version:", "(", version); 
}
...
bool ESP8266::sATCWJAP2(String ssid, String pwd)
{    
    String data;
    rx_empty();
...
bool ESP8266::eATCIFSR(String &list)
{
    ...
    //return recvFindAndFilter("OK", "\r\r\n", "\r\n\r\nOK", list);
    return recvFindAndFilter("STA", "\"", "\"", list);
}
